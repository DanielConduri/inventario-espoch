...
AuthenticationContext.prototype.login = function () {
  ...
  this._logstatus(
    "Expected state: " + expectedState + " startPage:" + window.location
  );
...
AuthenticationContext.prototype._renewIdToken = function (callback) {
  ...
  if (!this._hasResource(this.config.clientId)) {
    ...
    this._saveItem(
      this.CONSTANTS.STORAGE.TOKEN_KEYS,
      keys + this.config.clientId + this.CONSTANTS.RESOURCE_DELIMETER
    );
  }
  ...
  if (parameters) {
    ...
    if (
      ...
      if (
        ...
        var statesInParentContext =
          window.parent.AuthenticationContext()._renewStates;
  ...
  if (requestInfo.parameters.hasOwnProperty(this.CONSTANTS.ERROR_DESCRIPTION)) {
    ...
    if (requestInfo.requestType === this.REQUEST_TYPE.LOGIN) {
      ...
      this._saveItem(
        ...
        requestInfo.parameters.errorDescription
      );
    } else {
      this._renewActive = false;
    }
